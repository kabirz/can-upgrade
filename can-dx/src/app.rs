...
fn can_worker(rx: std::sync::mpsc::Receiver<CanCommand>) {
    ...
    for cmd in rx {
        match cmd {
            ...
            CanCommand::Connect { channel_idx } => {
                let devices = manager.detect_devices();
                if channel_idx >= devices.len() {
                    push_event(CanEvent::Error("无效的设备索引".into()));
                    continue;
                }
                let device_id = devices[channel_idx].id.clone();
...
pub fn App() -> Element {
    ...
    let mut selected_baud = use_signal(|| 3usize);
            ...
            for evt in events {
                match evt {
                    ...
                    CanEvent::Version(v) => {
                        ...
                        let patch = (v >> 8) & 0xFF;
                        version_str
                            .set(format!("固件版本: v{}.{}.{}", major, minor, patch));
                        is_version_querying.set(false);
                    }
                    CanEvent::FlashResult(_) => {
                        is_updating.set(false);
                        progress.set(0);
                    }
                    CanEvent::Error(e) => {
                        log_lines.write().push(format!("错误: {}", e));
                        is_updating.set(false);
                        is_version_querying.set(false);
                    }
                }
    ...
    let on_connect = move |_| {
        ...
        if *connected.read() {
            ...
            let _ = tx.send(CanCommand::Connect { channel_idx: idx });
        }
    };
        ...
        div {
            ...
            div {
                ...
                div {
                    ...
                    button {
                        style: if *is_updating.read() {
                            "padding: 6px 20px; border: none; border-radius: 4px; background: #999; color: white; font-weight: bold; cursor: not-allowed; margin-left: auto;"
                        } else {
                            "padding: 6px 20px; border: none; border-radius: 4px; background: #28a745; color: white; font-weight: bold; cursor: pointer; margin-left: auto;"
                        },
                        disabled: !can_flash,
                        onclick: on_flash,
                        if *is_updating.read() { "升级中..." } else { "开始刷写" }
                    }
                }
            ...
            fieldset {
                style: "border: 1px solid #ccc; border-radius: 6px; padding: 12px; background: white; flex: 1; display: flex; flex-direction: column; min-height: 0;",
                legend { "运行日志" }

                div {
                    style: "flex: 1; overflow-y: auto; background: #1e1e1e; color: #d4d4d4; padding: 8px; border-radius: 4px; font-family: 'Consolas', 'Courier New', monospace; font-size: 13px; white-space: pre-wrap; word-break: break-all; min-height: 150px;",
                    "{log_text}"
                }
